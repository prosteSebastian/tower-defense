...
namespace rang_implementation {
    ...
    inline void setWinSGR(rang::bg col, SGR &_state) noexcept
    {
        if (col != rang::bg::reset) {
            _state.bgColor = ansi2attr(static_cast<BYTE>(col) - 40);
        } else {
            _state.bgColor = defaultState().bgColor;
        }
    }

    inline void setWinSGR(rang::fg col, SGR &_state) noexcept
    {
        if (col != rang::fg::reset) {
            _state.fgColor = ansi2attr(static_cast<BYTE>(col) - 30);
        } else {
            _state.fgColor = defaultState().fgColor;
        }
    }

    inline void setWinSGR(rang::bgB col, SGR &_state) noexcept
    {
        _state.bgColor = (BACKGROUND_INTENSITY >> 4)
          | ansi2attr(static_cast<BYTE>(col) - 100);
    }

    inline void setWinSGR(rang::fgB col, SGR &_state) noexcept
    {
        _state.fgColor
          = FOREGROUND_INTENSITY | ansi2attr(static_cast<BYTE>(col) - 90);
    }

    inline void setWinSGR(rang::style style, SGR &_state) noexcept
    {
        switch (style) {
            case rang::style::reset: _state = defaultState(); break;
            case rang::style::bold: _state.bold = FOREGROUND_INTENSITY; break;
            case rang::style::underline:
            case rang::style::blink:
                _state.underline = BACKGROUND_INTENSITY;
                break;
            case rang::style::reversed: _state.inverse = TRUE; break;
            case rang::style::conceal: _state.conceal = TRUE; break;
            default: break;
        }
    }
    ...
    {
        static SGR _state = defaultState();
        return _state;
    }

    inline WORD SGR2Attr(const SGR &_state) noexcept
    {
        WORD attrib = 0;
        if (_state.conceal) {
            if (_state.inverse) {
                attrib = (_state.fgColor << 4) | _state.fgColor;
                if (_state.bold)
                    attrib |= FOREGROUND_INTENSITY | BACKGROUND_INTENSITY;
            } else {
                attrib = (_state.bgColor << 4) | _state.bgColor;
                if (_state.underline)
                    attrib |= FOREGROUND_INTENSITY | BACKGROUND_INTENSITY;
            }
        } else if (_state.inverse) {
            attrib = (_state.fgColor << 4) | _state.bgColor;
            if (_state.bold) attrib |= BACKGROUND_INTENSITY;
            if (_state.underline) attrib |= FOREGROUND_INTENSITY;
        } else {
            attrib = _state.fgColor | (_state.bgColor << 4) | _state.bold
              | _state.underline;
        }
        return attrib;
    }
