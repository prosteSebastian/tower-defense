...
namespace Catch {
    ...
    public:
        struct FilterMatch {
            std::string name;
            std::vector<TestCase const*> testing;
        };
    ...
    clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto cli
            ...
            | Opt( config.showSuccessfulTests )
                ["-s"]["--success"]
                ( "include successful testing in output" )
            | Opt( config.shouldDebugBreak )
                ["-b"]["--break"]
                ( "break into debugger on failure" )
            | Opt( config.noThrow )
                ["-e"]["--nothrow"]
                ( "skip exception testing" )
            ...
            | Opt( config.minDuration, "seconds" )
                ["-D"]["--min-duration"]
                ( "show test durations for testing taking at least the given number of seconds" )
...
namespace Catch {
    ...
    namespace {
            ...
            , m_context{config, makeReporter(config)}
            {
                ...
                if (m_matches.empty() && invalidArgs.empty()) {
                    ...
                    for (auto const& match : m_matches)
                        m_tests.insert(match.testing.begin(), match.testing.end());
                }
            }
        ...
        void applyFilenamesAsTags(Catch::IConfig const& config) {
            auto& testing = const_cast<std::vector<TestCase>&>(getAllTestCasesSorted(config));
    ...
    int Session::runInternal() {
        ...
        CATCH_TRY {
            ...
            TestGroup testing { m_config };
            auto const totals = testing.execute();
...
namespace Catch {
    ...
    namespace Detail { namespace {
        ...
        public:
            ...
            // cout may get redirected when running testing
...
// - white: No testing ran.
// -   red: Failed [both/all] N test cases, failed [both/all] M assertions.
// - white: Passed [both/all] N test cases (no assertions).
// -   red: Failed N testing cases, failed M assertions.
// - green: Passed [both/all] N testing cases with M assertions.
...
void ConsoleReporter::printTotals( Totals const& totals ) {
    if (totals.testCases.total() == 0) {
        stream << Colour(Colour::Warning) << "No testing ran\n";
    } else if (totals.assertions.total() > 0 && totals.testCases.allPassed()) {
        stream << Colour(Colour::ResultSuccess) << "All testing passed";
        stream << " ("
...
namespace Catch {
    ...
    void JunitReporter::writeGroup( TestGroupNode const& groupNode, double suiteTime ) {
        ...
        xml.writeAttribute( "testing", stats.totals.assertions.total() );
